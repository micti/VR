...
function convertToOldFormat(newFormatData) {
  ...
  for (const train of newFormatData.trains) {
...
// Xây dựng mô phỏng
const builder = async function (dataFile) {
  let o = await d3Json(dataFile);
  // convert to old format
  const data = !o.stations ? convertToOldFormat(o) : o;
...
const simConfig = {
  init: function (data) {
    simConfig.data = data;
    simConfig.scaleStation = scaleLinear()
    ...
    for (const d in simConfig.data.stations) {
...
const sim = {
  ...
  init: function (speed) {
    ...
    sim.speed = speed;
    for (const train in simConfig.data.routes) {
      for (let i = 0; i <= 4; i++) {
  ...
  render: function () {
    ...
    var svg = select("#bieudo")
      .append("svg")
      .attr("width", 250)
      .attr("height", height);

    // Cột KM
    var axisKM = axisLeft(simConfig.scaleStation).ticks(32);
  ...
  createTrain: function (name, route, day) {
    ...
    var i = 0;
    for (const key in simConfig.data.routes[route].sections) {
      var dayStart =
  ...
  resetLastPos: function () {
    for (const pos in sim.trainLastPos) {
  ...
  updatePerformance: function () {
    ...
    if (now - sim.performanceStats.lastFpsUpdate >= 1000) {
      ...
      var fpsElement = document.getElementById("fps_counter");
      if (fpsElement) {
        fpsElement.innerText = sim.performanceStats.fps + " FPS";
      }
    }
  },
...
const marley = {
  ...
  render: function () {
    // Tạo dữ liệu
    for (const route in simConfig.data.routes) {
    ...
    var bieudoStationSVG = select("#bieudo_chaytau_station_svg")
      ...
      .attr("width", 50)
      .attr("height", simConfig.lineLength * simConfig.scaleRatio + 50);
    var axisTime = axisTop(simConfig.scaleTime)
      .ticks(24)
  ...
  getTrainDiagramData: function (route) {
    ...
    var count = 0;
    for (const section in simConfig.data.routes[route].sections) {
  ...
  routeMouseOver: function (d, i) {
    select(this.parentNode).selectAll("text").style("opacity", 1);
    select(this.parentNode).selectAll("circle").style("opacity", 1);
    select(this.parentNode).moveToFront();
  },

  routeMouseOut: function (d, i) {
    select(this.parentNode).selectAll("text").style("opacity", 0);
    select(this.parentNode).selectAll("circle").style("opacity", 0);
  },
};
...
// Export the builder function and main objects
export { builder, simConfig, sim, marley, simControl };
