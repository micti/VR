...
var scaleStation = (scale = d3
  .scaleLinear()
  .domain([0, 1726])
  .range([0, 1726 * scaleRatio]));
var scaleTime = d3
  .scaleTime()
...
bieudoTimeSVG.append("g").attr("transform", "translate(60,20)").call(axisTime);
...
d3.json("/data/tet2017.json", function (data) {
  ...
  var setTrainByTime = function (index, time) {
    ...
    if (trainLastPos[index] === null) {
      ...
      do {
        if (
          trains[index].route[preRoutes].start <= time &&
          trains[index].route[preRoutes].out >= time
        ) {
          route = preRoutes;
          break;
        }
      ...
      do {
        if (
          trains[index].route[route].start <= time &&
          trains[index].route[route].out >= time
        ) {
          break;
        }
        route++;
    ...
    var routeDuration =
      trains[index].route[route].in - trains[index].route[route].start;
    ...
    var currentPos =
      data.stations[station1].km +
      (routeRunningTime * routeLenght) / routeDuration;
  ...
  d3.select("#schedule_train")
    .selectAll("div")
    .data(dataTrainCode)
    .enter()
    .append("div")
    .attr("class", function (d) {
      return "train t_" + d.split(" ")[0].toLowerCase();
    })
    .text(function (d) {
      return d;
    });
  ...
  w.on("scroll", function () {
    ...
    if (
      window_scroll_h > bieudo_mophong_offset_top &&
      window_scroll_h < bieudo_mophong_offset_top + bieudo_mophong_h - 30
    ) {
      $("#schedule_day").css(
        "top",
        window_scroll_h - bieudo_mophong_offset_top,
  ...
  ccDetail
    .append("div")
    .attr("class", function (d) {
      return "info t_" + d.code.split("_")[0].toLowerCase();
    })
    .text(function (d) {
      return d.code;
    });
  ...
  d3.selectAll(".submenu ul")
    ...
    .on("click", function () {
      ...
      if (_view === "viewTrainSchedule_DateStart")
        return viewTrainScheduleDateStart();
      ...
      if (_view === "viewTrainSchedule_Locomotive")
        return viewTrainScheduleLocomotive();
